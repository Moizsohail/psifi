...
class LoginRegisterPageState extends State<LoginRegisterPage>{
  ...
  final FirebaseMessaging _messaging = FirebaseMessaging(); //firebase messaging object created
  FirestoreHelper _firestore = FirestoreHelper("pushtokens"); //helper functions for pushtokens collection
  
  void fireMessagingListeners(){
    //requestNotificationsPermissions?
    _messaging.configure( //Firebase MessageHandler setup, functions to perform on specific actions
      onMessage: (message) async => print('onMessage $message'),
      onResume: (message) async => print('onResume $message'),
      onLaunch: (message) async => print('onLaunch $message')
    ); //simply printing for now
  }
  ...
  void initState() {
    super.initState();
      _messaging.getToken().then((token){
        print(token);
        _firestore.addUniqueData({"devtoken":token},"devtoken"); //adding a devtoken
      });
      fireMessagingListeners(); //firebase messaging listeners setup
  }

  Widget loginBtn(){ //dynamic flare animated login btn widget, plays anim on state change
  ...
  void validateAndSubmit(){
    setState(()=>_loginButtonState = LoginButtonState.loading); //loading state while given credentials are being validated and processed
  ...
  void moveToLogin(){
    ...
    setState((){ _formType = FormType.login;}); //formType set to login
  } 
  
  void moveToRegister(){ //switch to registration/signUp page
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      ...
      body: Container(
        ...
        child: Form( //key,child,autovalidate,onWillPop,onChanged
          key:formKey,
          child: ListView(
            children: <Widget>[
              ...
              FlatButton( //Form state toggle button
                //Text to display depending on the form selected.
                child: Text(_formType==FormType.login ? "Don't Have An Account? Register" : "Already Have An Account? Login"),
                onPressed: (_formType==FormType.login ? moveToRegister : moveToLogin), //toggle form state by the moveTo functions
              )
            ],
          ),
        ),
